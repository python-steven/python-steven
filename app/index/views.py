        ...
        try:
            ...
            if number != "All":
                ...
                if page_num >= page:
                    ...
                    dict_data['data'] = budget_statement_detail
                    dict_data['total_count'] = count
                    dict_data['current_page'] = page
                    dict_data['count_page'] = page_num
                    return restful.ok(data=dict_data)
                else:
...
def statement_query(request):
    if request.method == "POST":
        try:
            ...
            query_formId = request.POST['query_formId']
            id = request.session['user_Id']
            dict_data={}
            ...
            if query_formId != "":
                sql = sql + 'AND "FormId" = \''+query_formId +'\''
                sql2 = sql2 + 'AND "FormId" = \''+query_formId +'\''
            if query_status != "":
            ...
            if number != "All":
                ...
                if page <= count_page:
                    ...
                    dict_data['total_count']=count
                    dict_data['current_page']=page
                    dict_data['count_page']=count_page
                    return restful.ok(data=dict_data)
                else:
                    return restful.params_error(message="it had no other pages")
        except Exception as e:
