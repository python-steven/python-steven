    ...
    if request.method =="POST":
        try:
            ...
            if Next_maintain_time != "" and Next_maintain_time_1 !="":
                ...
                sql_count =  sql_count + ' AND ("NextCheckDate" >= \'' + Next_maintain_time + '\' AND "NextCheckDate" <= \''+Next_maintain_time_1+'\')'
            if Next_maintain_time !="" and Next_maintain_time_1 =="":
                sql = sql+' AND ("NextCheckDate" >= \''+Next_maintain_time+'\')'
                sql_count = sql_count+' AND  ("NextCheckDate" >= \''+Next_maintain_time+'\')'
            if Next_maintain_time_1 !=""and Next_maintain_time =="":
                sql = sql+' AND ("NextCheckDate" <= \''+Next_maintain_time_1+'\')'
                sql_count =  sql_count+' AND ("NextCheckDate" <= \''+Next_maintain_time_1+'\')'
            # if Next_maintain_time !="" and Next_maintain_time_1 =="":
            #     sql ='SELECT * FROM(' + sql+') as t1 WHERE "NextCheckDate" >= \''+Next_maintain_time+'\''
